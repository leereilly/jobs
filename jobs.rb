...
def generate_readme
  ...
  github_jobs.each do |job|
    job_id = job_id + 1
    markdown << "<tr><td>**<a title='Go to #{job['company_url']} homepage' href='#{job['company_url']}'>#{job['company']}</a>**</td><td><a id='#{job_id}' title='Go to #{job['title']} description' href='#{job['url']}'>#{job['title']}</a></td><td><img src='#{job['company_logo']}' height='25'></td></tr>\n"
  end
  markdown << "<table>"
  return markdown
end
